...
""";
    ...
    public partial record LocationWrap : IDeserialize<LocationWrap, Location>
    {
        ...
        private sealed class SerdeVisitor : Serde.IDeserializeVisitor<Benchmarks.Location>
        {
            ...
            private sealed class FieldNameVisitor : Serde.IDeserializeState<byte>, Serde.IDeserializeVisitor<byte>
            {
                public byte DeserializeState<D>(ref D deserializer) where D : IDeserializer
                    => deserializer.DeserializeString<byte, FieldNameVisitor>(this);
            ...
            Benchmarks.Location Serde.IDeserializeVisitor<Benchmarks.Location>.VisitDictionary<D>(ref D d)
            {
                ...
                while (d.TryGetNextKey(visitor, out var key))
                {
                    switch (key)
                    {
                        case 1:
