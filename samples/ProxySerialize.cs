...
sealed partial class VersionSerdeObj : ISerde<Version>
{
    ...
    public void Serialize(Version version, ISerializer serializer)
    {
        ...
        var ser = SerializeProvider.GetSerialize<VersionProxy>();
        ser.Serialize(proxy, serializer);
    }

    public Version Deserialize(IDeserializer deserializer)
    {
        var de = DeserializeProvider.GetDeserialize<VersionProxy>();
        var proxy = de.Deserialize(deserializer);
